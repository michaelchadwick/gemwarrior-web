...
class Modal {
  ...
  _createModal(modalType) {
    ...
    if (modalType != 'temp') {
      ...
      const permTypes = ['perm', 'perm-debug', 'perm-win']

      // Close
      if (permTypes.includes(modalType)) {
    ...
    const confirmTypes = ['confirm', 'confirm-debug', 'confirm-start']

    // if a confirm modal, add buttons
    if (confirmTypes.includes(modalType)) {
      if (modalType == 'confirm-start') {
        this.modal.classList.add('confirm-start')
      }
